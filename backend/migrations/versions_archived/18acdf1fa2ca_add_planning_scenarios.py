...
def upgrade():
    ...
    op.create_table('scenario_allocations',
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('scenario_id', sa.UUID(), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.CheckConstraint("(kind = 'fixed' AND value >= 0) OR (kind = 'percent' AND value BETWEEN 0 AND 100)", name='ck_alloc_value_semantics'),
    sa.ForeignKeyConstraint(['scenario_id'], ['planning_scenarios.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('scenario_allocations', schema=None) as batch_op:
        ...
        batch_op.create_index(batch_op.f('ix_scenario_allocations_scenario_id'), ['scenario_id'], unique=False)
