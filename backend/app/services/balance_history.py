...
from app.models import Account, Transaction, AccountHistory
...
def calculate_daily_balances(
    ...
    logger.info(f"Calculating balances for {account_id} from {start_date} to {end_date}")
...
def store_balance_history(account_id: str, balance_records: List[Dict]) -> int:
    ...
    existing_records = (
        ...
        .filter(AccountHistory.account_id == lookup_key)
        .all()
    )
    ...
    try:
        ...
        logger.info(f"Processed {total_processed} balance history records for {account_id} ({stored_count} new, {updated_count} updated)")
...
def update_account_balance_history(
    ...
    try:
        ...
        if not force_update:
            latest_record = (
                AccountHistory.query
                .filter_by(account_id=lookup_key)
        ...
        logger.info(f"Updating balance history for {account.name} ({account.id}/{account.account_id}) from {start_date} to {end_date}")
        logger.info(f"Current balance: {account.balance} -> normalized: {current_balance}")
...
def get_balance_history_from_db(
    account_id: str, 
    days: int = 30
) -> List[Dict]:
    """
