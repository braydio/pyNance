...
- **Forecast recompute API** (`POST /api/forecast/compute`) with moving-average windows, normalization toggles, graph modes, distributed manual adjustments, and a typed `series` object for realized income, manual adjustments, spending, and debt totals.
