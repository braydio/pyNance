...
def filter_documents(
    ...
    for document, id in tqdm(
        zip(documents, ids), total=len(documents), desc="Filtering documents"
    ):
        labels[id] = {}
...
def retrieve_document_filter_batch(
    client: AnthropicClient, batch_id: str
...
def retrieve_document_filter_batch_df(
    ...
    result_df = pd.DataFrame(
        {"id": ids, "criterion": criteria, "classification": classification}
    )
...
def create_golden_dataset(
    ...
    for id, document in tqdm(
        zip(ids, documents), total=len(ids), desc="Generating queries"
    ):
        PROMPT = f"""
...
def retrieve_batch(client: AnthropicClient, batch_id: str) -> pd.DataFrame:
    batch = client.messages.batches.results(batch_id)
...
def create_naive_query_batch(
    client: AnthropicClient, model: str, documents: List[str], ids: List[str]
) -> str:
    if len(ids) != len(documents):
        raise ValueError("Length of ids must match length of documents")
