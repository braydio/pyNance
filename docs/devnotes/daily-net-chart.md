...
- **Tooltip.** The hover tooltip uses the same treatment as the categories chart: theme card background (`--theme-bg`), yellow outline (`--color-accent-yellow`), and `Fira Code`-style typography. It presents **Net** as the primary line first, then Income, Expenses, and Transaction count as supporting details. When the comparison overlay is active and a prior value exists, the tooltip also shows the prior value plus a `vs prior` delta with percent change when calculable (while suppressing noisy percent text for divide-by-zero cases). The card snaps to the yellow net dash.
