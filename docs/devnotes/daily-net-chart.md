...
- **Tooltip.** The hover tooltip uses the same treatment as the categories chart: theme card background (`--theme-bg`), yellow outline (`--color-accent-yellow`), and tuned `Fira Code`-style typography/padding for title/body. It lists Net first, then Income, Expenses, and Transaction count. When the comparison overlay is active, it adds the prior value and an inline `vs prior` delta/percent summary when a non-zero baseline supports percent math. The card remains snapped to the yellow net dash.
