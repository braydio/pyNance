...
- **Trendline colors.** Moving-average and average lines follow the accent palette: the 7-day trend uses the purple accent (`--color-accent-purple`), the 30-day trend leans on cyan (`--color-accent-cyan`), the income average matches the cyan highlight, and the expense average keeps the red tone. These lines should remain thin, transparent-filled strokes with no visible points.
...
- **Tooltip.** The hover tooltip uses a black background with a yellow border (`borderColor` matches the net accent). It lists Income, Expenses, Net, and Transaction count, with the date as the title. When the comparison overlay is enabled, add a simple “Prior …” block below the primary values showing the prior-period income/expenses/net for that label.
