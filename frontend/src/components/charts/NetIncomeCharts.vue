<template>
  <div class="net-chart">
    ...
    <div class="chart-summary">
      <div class="summary-line income">Income: ${{ formatAmount(summary.totalIncome) }}</div>
      <div class="summary-line expenses">Expenses: ${{ formatAmount(summary.totalExpenses) }}</div>
      <div class="summary-line net">Net: ${{ formatAmount(summary.totalNet) }}</div>
    </div>

    <div class="chart-controls">
      <button @click="changeGranularity('daily')" :class="{ active: granularity === 'daily' }">
        Daily
      </button>
    ...
    const updateChart = async () => {
      ...
      chartInstance.value = new Chart(ctx, {
        ...
        options: {
          ...
          scales: {
            x: {
              ticks: { color: getStyle('--color-text-light') },
              grid: { color: getStyle('--divider') },
            },
            y: {
              beginAtZero: true,
              ticks: { callback: (value) => `$${value}`, color: getStyle('--color-text-light') },
            },
          },
          plugins: {
            tooltip: {
